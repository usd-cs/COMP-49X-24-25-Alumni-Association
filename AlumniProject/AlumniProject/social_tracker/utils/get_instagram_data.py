...
def get_instagram_stories(access_token):
    ...
    # Use the /me/stories endpoint directly
    stories_url = f"https://graph.instagram.com/v19.0/me/stories"
    stories_params = {
        "fields": "id,timestamp,permalink", # Add any other base fields needed
        "access_token": access_token
    }
    ...
    try:
        ...
        # Limit printing potentially large response data
        ...
        if "data" in data and len(data["data"]) > 0:
            ...
            for story in stories:
                ...
                # Initialize default story metrics
                story_metrics = {
                ...
                if timestamp_str:
                    try:
                        ...
                        print(f"Error parsing timestamp {timestamp_str} for story {story_id}: {date_err}")
                        # Keep date_posted as None or handle differently
                else:
                     print(f"Missing timestamp for story {story_id}")

                # --- Fetch Insights (remains largely the same) ---
                try:
                    ...
                    # print(f"Requesting metrics: {insights_params['metric']}") # Can be verbose
                    
                    insights_response = requests.get(insights_url, params=insights_params)
                    # print("\nResponse Status: " + str(insights_response.status_code)) # Can be verbose
                    
                    insights_data = insights_response.json()
                    # print("\nRaw Response Data: " + json.dumps(insights_data, indent=2)) # Can be verbose

                    if insights_response.status_code != 200 or "error" in insights_data:
                        error_detail = insights_data.get("error", {}).get("message", "Unknown insights error")
                        print(f"\nError fetching insights for story {story_id}: {error_detail}")
                        # Append story with basic info even if insights fail
                        active_stories_data.append(story_metrics)
                        print(f"Story {story_id} added with basic information only (insights failed)")
                    ...
                    if "data" in insights_data and insights_data["data"]:
                        ...
                        # print("\nProcessed metrics for story " + str(story_id) + ":", json.dumps(metrics, indent=2)) # Can be verbose
                        ...
                        # print("\nMetrics updated for story dictionary:") # Can be verbose
                        # print(f"Views: {story_metrics['num_views']}, Clicks: {story_metrics['num_profile_clicks']}, Swipes: {story_metrics['num_swipes_up']}")
                        
                    # Append the final story data (with or without metrics) to the list
                    active_stories_data.append(story_metrics)
                    print(f"Successfully processed story {story_id}")

                except Exception as e:
                    print(f"Could not process insights for story {story_id}: {str(e)}")
                    # Avoid duplicates if error happened after appending basic info
                    if story_id not in [s.get('story_API_ID') for s in active_stories_data]: 
                         active_stories_data.append(story_metrics) 
                    print(f"Story {story_id} added with basic information only due to exception")
                    continue # Move to next story

            return active_stories_data # Return the list of story dictionaries
        ...
        print(f"Raw response text: {response.text}") # Log raw text
